...
class CampusWrapper():
    def __init__(self, building, floor, room, data = FileNotFoundError) -> None:
...
class BookingSystem:
    ...
    def book_room(self, index: int, start_time: int, booker_name: str, booking_type: str) -> None:
        ...
        if booking is not None: #access_at_index method has return type -> T | None. This is to ensure it accounts for that.
            booking.hourly_bookings[start_time].booker_name = booker_name
            booking.hourly_bookings[start_time].booking_type = booking_type
    ...
    def delete_booking(self, index: int, start_time: int) -> None:
        ...
        if bookingToDelete is not None: #same as above
            booking_search.delete(avl.hash_str_to_int(bookingToDelete.hourly_bookings[start_time].booker_name))
            bookingToDelete.hourly_bookings[start_time].booker_name = "None"
            bookingToDelete.hourly_bookings[start_time].booking_type = "Vacant"
...
class HourlyBooking:
    ...
    def update_booking(self, booker_name: str, booking_type: str, campus_info) -> None:
        if booker_name == "None":
